...
include "include/db.php";  
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") 
    {
        ...
        if (!empty($_POST["userPassword"]) && !empty($_POST["userRepeatPassword"]) && !empty($_POST["userMail"]) && testpwmatch($_POST["userPassword"], $_POST["userRepeatPassword"]))
        {
            ...
            { 
                if($db->query("INSERT INTO users (UEmail, UPassword, UFirstname, ULastname, UAddress, UPostCode, UCity, UPhone, UCountry, UPriviledge)
                VALUE ('{$_POST['userMail']}', '{$_POST['userPassword']}', '{$_POST['userFirstname']}', '{$_POST['userLastname']}', '{$_POST['userAddress']}',
                       '{$_POST['userPostcode']}', '{$_POST['userCity']}', '{$_POST['userPhone']}', '{$_POST['userCountry']}', 'User')"))
                {
...
<meta charset="UTF-8">
    ...
    <title>Weirdplantgirl | New user</title>
</head>
<body>
<?php
